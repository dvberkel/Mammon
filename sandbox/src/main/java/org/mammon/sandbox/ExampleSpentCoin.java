...
public class ExampleSpentCoin<G extends Group<G>, S, T, H extends SignatureHashFunction<G>, H0 extends PaymentHashFunction<G, S, T>>
	...
	private final BigInteger[] spendingCommitments;
	...
	public ExampleSpentCoin(UnspentCoin<G, S, T, H, H0> basedOnCoin, ExampleBank<G, S, T, H, H0> bank,
		...
		// Generated by shop
		BigInteger d = setup.getPaymentHash().hash(blindedIdentity, commitment, bearer.getIdentity(), time);

		// Generated by account holder
		spendingCommitments = (BigInteger[]) Array.newInstance(BigInteger.class, 2);
		spendingCommitments[0] = d.multiply(basedOnCoin.getBearer().getPrivateKey()).multiply(
				basedOnCoin.getBlindingFactor()).multiply(basedOnCoin.getPayerWitness()[0]);
		spendingCommitments[1] = d.multiply(basedOnCoin.getBlindingFactor()).multiply(basedOnCoin.getPayerWitness()[1]);
	...
	@Override
	public BigInteger[] getSpendingCommitments() {
